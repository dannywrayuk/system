...
local addFileLines = function(buf, startLine, values)
    for index, value in ipairs(values) do
        ...
        nextChar = highlightSection(buf, config.highlights.file.name, startLine + index - 1, nextChar + 1, #value.file.fileName)
        nextChar = highlightSection(buf, config.highlights.file.path, startLine + index - 1, nextChar + 1, #value.file.path)
    end
