...
local bufnmap = function (buffer, map, callback)
    vim.api.nvim_buf_set_keymap(buffer, "n", map, "", { callback = callback })
end
...
M.openWindow = function()
    ...
    if buf == nil then
        ...
        vim.api.nvim_create_autocmd('BufEnter', {
            buffer = buf,
            callback = function()
                bufnmap(buf, config.commands.refresh, function ()
                    updateContent(buf)
                end)
                bufnmap(buf, config.commands.toggleCurrent, function ()
                    staging.toggleCurrent()
                    updateContent(buf)
                end)
                bufnmap(buf, config.commands.toggleAll, function ()
                    staging.toggleAll()
                    updateContent(buf)
                end)
                bufnmap(buf, config.commands.addAll, function ()
                    staging.addAll()
                    updateContent(buf)
                end)
                bufnmap(buf, config.commands.restoreAll, function ()
                    staging.restoreAll()
                    updateContent(buf)
                end)
                bufnmap(buf, config.commands.discardCurrent, function ()
                    staging.discardCurrent()
                    updateContent(buf)
                end)
                bufnmap(buf, config.commands.discardAll, function ()
                    staging.discardAll()
                    updateContent(buf)
                end)
                bufnmap(buf, "<esc>", M.closeWindow)
